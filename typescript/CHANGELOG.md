...
* **verify:** match action names case-insensitively (culture-invariant) — stop false `unbounded_action` violations on allowed actions (e.g. `Edit` vs bounded `edit`). Both sides folded with `toLowerCase()`/`str.lower()` (Unicode default, locale-independent) on the exact-match and colon-prefix paths, across the TS and Python SDKs. Backport of the fix on `main` (2.0.x) to the 1.3.x line consumed by the platform observer.
