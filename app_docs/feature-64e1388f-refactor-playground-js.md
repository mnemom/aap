...
- Fixed a latent bug in `switchVizView` where active-panel logic was hardcoded to `timeline`/`matrix` strings, replaced with a generic `panel.id === \`viz-${view}\`` pattern that correctly handles any future view names
...
- `docs/playground/playground.js`: Refactored `setupExampleSelectors`, added `renderItemList`, updated `displayVerifyResult`, `displayCoherenceResult`, `displayDriftResult`, and `switchVizView`
...
- **`switchVizView`**: Active-panel detection changed from `panel.id === 'viz-timeline'` + boolean arithmetic to `panel.id === \`viz-${view}\``, which is self-consistent with the panel naming convention and open to extension.
...
- The `switchVizView` fix closes a latent extensibility issue: adding a third visualization tab previously would have required editing the boolean logic; now it works automatically.
